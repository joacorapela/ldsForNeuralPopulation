...
if ~LONO.do % default
    savedir = fullfile(resultdir,animalname,['Joint_trial_based_split_',num2str(splitDelays)],[num2str(binSizems),'ms','Bins']);
    savename = ['Joint','_PLDSfitRes_',datestr(now,'yy_mm_dd_HH_MM_SS')];
else
%     LONO.file = ...
...
[seq,N_V1,N_LM] = buildTrialBasedSeq_Joint(summarymatfile, binSizems,binWinms,LONO,splitDelays);
...
if numel(nStates) == 1 % evaluatig single model
    [params ,seq ,varBound ,EStepTimes ,MStepTimes] = dofitWithNstates_Joint(nStates,seq,Inference_handle,config,N_V1,N_LM);
