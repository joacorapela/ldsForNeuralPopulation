...
if ~LONO.do % default
    savedir = fullfile(resultdir,animalname,['Joint_trial_based_splitDynamics'],[num2str(binSizems),'ms','Bins']);
    savename = ['JointSep_lag',num2str(lag),'_PLDSfitRes_',datestr(now,'yy_mm_dd_HH_MM_SS')];
else
%     LONO.file = ...
...
splitDelays = 0;
[seq,N_V1,N_LM] = buildTrialBasedSeq_JointSep(summarymatfile, binSizems,binWinms,LONO,splitDelays,lag);
...
if numel(nStates) == 1 % evaluatig single model
    [params ,seq ,varBound ,EStepTimes ,MStepTimes] = dofitWithNstates_JointSep(nStates,seq,Inference_handle,config,N_V1,N_LM);
