function [seq,N_V1,N_LM] = buildTrialBasedSeq_JointSepCon_CntrlOnly(summarymatfile, binSizems,binWinms,LONO,splitDelays,perffile,trialType,Xval)
...
%%
% function [seq,N_V1,N_LM] = buildTrialBasedSeq_JointSepCon_CntrlOnly(summarymatfile, binSizems,binWinms,LONO,splitDelays,perffile,trialType)
% res = load(fullfile(summarymatfile.folder,summarymatfile.name));
% if ~isnan(binWinms)
%     error('custized window for trial, not implemented yet')
% end
...
%     elseif strcmp(trialType,'exGrooming')
%         validTrIdx = res.nogroomingind;
%     elseif strcmp(trialType,'exGrooming_go')
%         validTrIdx = intersect(res.gotrialind,res.nogroomingind);
%     elseif strcmp(trialType,'exGrooming_nogo')
%         validTrIdx = intersect(res.nogotrialind,res.nogroomingind);
%     else
%         error('trial type not implemented')
%     end
%     res = assignToFold(validTrIdx,res);
...
%      end
% 
%      if ~numel(LaserTrInd) % other option for when only using cntrl trials
%          seq(Tr).u(1,:) = pdtrace;
%       
%      else
%          Discard = [Discard,Tr];
